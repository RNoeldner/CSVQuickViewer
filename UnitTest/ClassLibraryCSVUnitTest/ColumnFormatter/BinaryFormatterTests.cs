...
namespace CsvTools.Tests
{
  ...
  public class BinaryFormatterTests
  {
    ...
    [TestMethod()]
    public void FoldersAbsolutePathTest()
    {
      var list = new[] { new Column("Name", new ValueFormat(DataTypeEnum.Binary, readFolder: "Test")) };
      var res = BinaryFormatter.FoldersAbsolutePath(list, UnitTestStatic.ApplicationDirectory).First();
      Assert.AreEqual(list[0].Name, res.Name);
      Assert.AreEqual(Path.Combine(UnitTestStatic.ApplicationDirectory, list[0].ValueFormat.ReadFolder), res.ValueFormat.ReadFolder);
    }
    [TestMethod()]
    public void FoldersRealtivePathTest()
    {
      var list = new[] { new Column("Name", new ValueFormat(DataTypeEnum.Binary, readFolder: Path.Combine(UnitTestStatic.ApplicationDirectory,"Test"))) };
      var res = BinaryFormatter.FoldersRealtivePath(list, UnitTestStatic.ApplicationDirectory).First();
      Assert.AreEqual(list[0].Name, res.Name);
      Assert.AreEqual("Test", res.ValueFormat.ReadFolder);
    }
  }
}