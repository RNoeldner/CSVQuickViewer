...
namespace CsvTools.Tests
{
  ...
  public class ManifestDataTests
  {
    ...
    public async Task ReadManifestAsync()
    {
      ...
      Assert.AreEqual(false, setting.Item1.HasFieldHeader);
      Assert.AreEqual(19, setting.Item2.Count());
      using (var reader = new CsvFileReader(setting.Item1.CsvFile(setting.Item2), null))
      {
    ...
    public async Task ReadManifestZip()
    {
      ...
      Assert.AreEqual(false, setting.Item1.HasFieldHeader);
      Assert.AreEqual(3, setting.Item2.Count());
      using (var reader = new CsvFileReader(setting.Item1.CsvFile(setting.Item2), null))
      {
        await reader.OpenAsync(UnitTestInitializeCsv.Token);
