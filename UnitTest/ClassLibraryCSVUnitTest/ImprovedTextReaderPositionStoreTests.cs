...
namespace CsvTools.Tests
{
  ...
  public class ImprovedTextReaderPositionStoreTests
  {
    ...
    public void ImprovedTextReaderPositionStoreTestStart()
    {
      using (var impStream = ImprovedStream.OpenRead(UnitTestInitialize.GetTestPath("txTranscripts.txt")))
      {
        using (var test = new ImprovedTextReader(impStream))
        {
          ...
          Assert.AreEqual(@"GCS_002846_Benavides	A23c25d3-3420-449c-a75b-0d74d29ddc38	Completed	13/03/2008 00:00:00	13/03/2008 00:00:00	13/03/2008 00:00:00", lastLine);
        }
      }
    }
    ...
    public void ImprovedTextReaderPositionStoreTestStartAsync()
    {
      using (var impStream = ImprovedStream.OpenRead(UnitTestInitialize.GetTestPath("txTranscripts.txt")))
      {
        using (var test = new ImprovedTextReader(impStream))
        {
          var store = new ImprovedTextReaderPositionStore(test);
          Assert.AreEqual(1, test.LineNumber);
          Assert.AreEqual("#UserID	CurriculumID	TranscriptStatus	RequestDateTime	RegistrationDateTime	CompletionDateTime", test.ReadLineAsync().Result);
          var lastLine = string.Empty;
          while (!store.AllRead)
          {
            lastLine = test.ReadLineAsync().Result;
          }
          Assert.AreEqual(@"GCS_002846_Benavides	A23c25d3-3420-449c-a75b-0d74d29ddc38	Completed	13/03/2008 00:00:00	13/03/2008 00:00:00	13/03/2008 00:00:00", lastLine);
        }
      }
    ...
    public void ImprovedTextReaderPositionStoreTestFromMiddle()
    {
      using (var impStream = ImprovedStream.OpenRead(UnitTestInitialize.GetTestPath("txTranscripts.txt")))
      {
        using (var test = new ImprovedTextReader(impStream, 65001, true, 1))
        {
          Assert.AreEqual(2, test.LineNumber);
          Assert.AreEqual(@"GCS_004805_Osipova	023c25d3-3420-449c-a75b-0d74d29ddc38	Completed	04/02/2008 00:00:00	04/02/2008 00:00:00	04/02/2008 00:00:00", test.ReadLine());
