...
namespace CsvTools.Tests
{
	...
	public class CsvHelperTest
	{
		...
		{
			using (var improvedStream = FunctionalDI.OpenStream(new SourceAccess(UnitTestInitializeCsv.GetTestPath("BasicCSV.txt"))))
				Assert.AreEqual(",", (await CsvHelper.GuessDelimiterFromStream(improvedStream, -1, 0, string.Empty, UnitTestInitializeCsv.Token)).Item1);
			...
			using (var improvedStream = FunctionalDI.OpenStream(new SourceAccess(test)))
				Assert.AreEqual("|", (await CsvHelper.GuessDelimiterFromStream(improvedStream, 20127, 10, string.Empty, UnitTestInitializeCsv.Token)).Item1);
		}
		...
		public async Task GuessDelimiterQualifierAsync()
		{
			...
			test.FileFormat.EscapeCharacter = string.Empty;
			using (var improvedStream = FunctionalDI.OpenStream(new SourceAccess(test)))
				Assert.AreEqual(",", (await CsvHelper.GuessDelimiterFromStream(improvedStream, test.CodePageId, test.SkipRows, test.FileFormat.EscapeCharacter, UnitTestInitializeCsv.Token)).Item1);
		}
		...
		public async Task GuessDelimiterTabAsync()
		{
			...
			test.FileFormat.EscapeCharacter = "\\";
			using (var improvedStream = FunctionalDI.OpenStream(new SourceAccess(test)))
				Assert.AreEqual("TAB", (await CsvHelper.GuessDelimiterFromStream(improvedStream, test.CodePageId, test.SkipRows, test.FileFormat.EscapeCharacter, UnitTestInitializeCsv.Token)).Item1);
		}
		...
		public async Task GuessNewlineTest()
		{
			...
			try
			{
				...
				using (var improvedStream = FunctionalDI.OpenStream(new SourceAccess(test)))
					Assert.AreEqual(RecordDelimiterType.LF, await CsvHelper.GuessNewlineFromStream(improvedStream, test.CodePageId, test.SkipRows, test.FileFormat.FieldQualifier, UnitTestInitializeCsv.Token));
				...
				using (var improvedStream = FunctionalDI.OpenStream(new SourceAccess(test)))
					Assert.AreEqual(RecordDelimiterType.RS, await CsvHelper.GuessNewlineFromStream(improvedStream, test.CodePageId, test.SkipRows, test.FileFormat.FieldQualifier, UnitTestInitializeCsv.Token));
				...
				using (var improvedStream = FunctionalDI.OpenStream(new SourceAccess(test)))
					Assert.AreEqual(RecordDelimiterType.LFCR, await CsvHelper.GuessNewlineFromStream(improvedStream, test.CodePageId, test.SkipRows, test.FileFormat.FieldQualifier, UnitTestInitializeCsv.Token));
				...
				using (var improvedStream = FunctionalDI.OpenStream(new SourceAccess(test)))
					Assert.AreEqual(RecordDelimiterType.CRLF, await CsvHelper.GuessNewlineFromStream(improvedStream, test.CodePageId, test.SkipRows, test.FileFormat.FieldQualifier, UnitTestInitializeCsv.Token));
			}
			finally
			{
				FileSystemUtils.FileDelete(path);
