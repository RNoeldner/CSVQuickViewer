  ...
  public class JsonFileReaderTest
  {
    ...
    public async Task OpenJsonArray()
    {
      ...
      using var jfr = new JsonFileReader(setting.FullPath, setting.ColumnCollection, setting.RecordLimit, setting.TrimmingOption == TrimmingOptionEnum.All,
        setting.TreatTextAsNull, setting.TreatNBSPAsSpace, m_TimeZoneAdjust, TimeZoneInfo.Local.Id, dpd);
    ...
    {
      var dpd = new Progress<ProgressInfo>();
      using var jfr = new JsonFileReader(UnitTestStatic.GetTestPath("Emp.json"), null, 0, false, string.Empty, false, StandardTimeZoneAdjust.ChangeTimeZone, TimeZoneInfo.Local.Id, dpd);
      await jfr.OpenAsync(UnitTestStatic.Token);
      try
      {
        _ = jfr.GetFloat(0);
    ...
    public async Task ReadJSon1Async()
    {
      ...
      using var jfr = new JsonFileReader(setting.FullPath, setting.ColumnCollection, setting.RecordLimit, setting.TrimmingOption == TrimmingOptionEnum.All,
        setting.TreatTextAsNull, setting.TreatNBSPAsSpace, m_TimeZoneAdjust, TimeZoneInfo.Local.Id, processDisplay);
      await jfr.OpenAsync(UnitTestStatic.Token);
