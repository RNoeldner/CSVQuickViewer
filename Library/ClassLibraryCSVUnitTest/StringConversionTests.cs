    ...
    public void IdentifyDatesFormatsUS()
    {
      ...
      foreach (var fmt in StringConversion.StandardDateTimeFormats)
      {
        ...
        for (var day = 5; day < 25; day++)
          for (var hrs = 10; hrs < 13; hrs++)
            samples.Add(new DateTime(2010, 5, day, hrs, 10, 27, 6).ToString(fmt, culture));

        Assert.IsNotNull(StringConversion.CheckDate(samples, fmt, dateSep, ":").FoundValueFormat,
