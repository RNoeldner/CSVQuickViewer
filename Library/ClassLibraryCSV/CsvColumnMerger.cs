  ...
  public sealed class CsvColumnMerger
  {
    ...
    in string rawText)
    {
      ...
      while (columns.Count > m_ExpectedColumns)
      {
        ...
        if (bestIndex < 1)
          break;
        columns[bestIndex-1] = MergeUsingRawText(columns, bestIndex, rawText);
    ...
    /// <summary>
    /// Merges two adjacent columns from a CSV row using the original raw CSV line
    /// rather than a fixed delimiter. This preserves the exact content of the columns,
    /// including any embedded delimiters, quotes, or spacing that might exist in the raw text.
    /// </summary>
    /// <param name="columns">The list of already-split column values for the row.</param>
    /// <param name="mergeIndex">
    /// The index of the column to merge into its left neighbor (i.e., columns[mergeIndex - 1]).
    /// </param>
    /// <param name="rawText">The original CSV line for the row, before splitting into columns.</param>
    /// <returns>
    /// A new array of strings with the specified columns merged based on their positions in <paramref name="rawText"/>.
    /// The resulting array has one fewer element than <paramref name="columns"/>.
    /// If the column positions cannot be determined in <paramref name="rawText"/>, the method falls back to a simple delimiter-based merge.
    /// </returns>
    private string MergeUsingRawText(IReadOnlyList<string> columns, int mergeIndex, string rawText)
    {
      ...
      // In case we could not find the column text, fall back to simple delimiter based merge
      return (starts[mergeIndex - 1]!=-1 && starts[mergeIndex+1]!=-1)
        ? rawText.Substring(starts[mergeIndex - 1], starts[mergeIndex + 1] - starts[mergeIndex - 1] -1)
        : columns[mergeIndex - 1] + m_Delimiter + columns[mergeIndex];
    }
...
#if DEBUG
      Debugger.Log(1, "Debug", sb.ToString());
#endif
      return score;
    }
