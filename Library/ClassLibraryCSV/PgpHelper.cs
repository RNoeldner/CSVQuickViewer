...
namespace CsvTools
{
  public static class PgpHelper
  {
    ...
    public static async Task DecryptFileAsync(this PgpSecretKeyRingBundle pgpSec, SecureString passPhrase,
    ...
    /// </exception>
    public static Stream GetReadStream(this PgpSecretKeyRingBundle pgpSec, in SecureString passPhrase, Stream inputStream,
      out Stream closeSecond, out Stream closeThird)
    {
      ...
      foreach (var pgpEncryptedData in encryptedDataList.GetEncryptedDataObjects())
      {
        ...
        try
        {
          var key = pgpSecKey.ExtractPrivateKey(passPhrase.GetText().ToCharArray());
